...
def DataSourceRuleInfo(data_source_rule_vo: DataSourceRule, minimal=False):
    ...
    if not minimal:
        info.update(
            {
                "conditions": DataSourceRuleConditionsInfo(
...
def DataSourceRulesInfo(data_source_rule_vos, total_count, **kwargs):
    return data_source_rule_pb2.DataSourceRulesInfo(
        results=list(
            map(functools.partial(DataSourceRuleInfo, **kwargs), data_source_rule_vos)
        ),
